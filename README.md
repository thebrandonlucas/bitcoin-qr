...
| `prefer-lightning` | `boolean` | **Optional**. If set and both `bitcoin` and `lightning` params are given, will use `lightning:` as the URI protocol prefix. This will open lightning-compatible wallets as opposed to on-chain wallets. | `false` |
| `imageEmbedded`    | `boolean` | **Optional**. Specify if the image should be embedded within the QR code or overlayed on top. The larger the image overlay, the higher the qr-error-correction-level needed                             | `false` |
